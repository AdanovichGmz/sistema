...
<meta name="mobile-web-app-capable" content="yes">
    ...
    $etequery1 = "SELECT COALESCE((SELECT  IFNULL(SUM( TIME_TO_SEC( tiempoTiraje) ),0)  FROM tiraje WHERE id_sesion=$id_sesion AND fechadeldia_tiraje = '$today' )+(SELECT  IFNULL(SUM( TIME_TO_SEC( tiempo_ajuste)),0) FROM tiraje WHERE id_sesion=$id_sesion AND fechadeldia_ajuste = '$today' )) as tiempo_real";

    //obtenemos el tiempo muerto sumando las idas al sanitario
    $etequery2 = "SELECT  IFNULL(SUM( TIME_TO_SEC(breaktime)),0)+(SELECT IFNULL(SUM(TIME_TO_SEC(tiempo_muerto)),0) FROM tiempo_muerto WHERE id_sesion=$id_sesion AND fecha = '$today') AS tiempo_muerto  FROM breaktime WHERE id_estacion=$stationID AND radios='Sanitario' AND fechadeldiaam = '$today' AND id_usuario =".$_SESSION['idUser'];
    $tmuerto_alertas=mysqli_fetch_assoc($mysqli->query("SELECT (SELECT  IFNULL(SUM( TIME_TO_SEC( tiempoalertamaquina) ),0)  FROM alertamaquinaoperacion WHERE id_estacion=$stationID AND fechadeldiaam = '$today' AND es_tiempo_muerto NOT IN('false') AND id_usuario=".$_SESSION['idUser'].") + (SELECT  IFNULL(SUM( TIME_TO_SEC(tiempoalertamaquina) ),0) FROM alertageneralajuste WHERE id_estacion=$stationID AND fechadeldiaam = '$today' AND es_tiempo_muerto NOT IN('false') AND id_usuario=".$_SESSION['idUser'].") AS tmuerto_alert"));
    
    //obtenemos la calidad a la primera operando entregados-defectos*100/cantidadpedida  
    $etequery3 = "SELECT COALESCE(((SELECT SUM(entregados)-SUM(merma_entregada) FROM tiraje WHERE id_sesion=$id_sesion AND fechadeldia_tiraje = '$today')-(SELECT SUM(defectos) FROM tiraje WHERE id_sesion=$id_sesion AND fechadeldia_tiraje = '$today' AND tiempoTiraje IS NOT NULL))/(SELECT SUM(entregados)-SUM(merma_entregada) FROM tiraje WHERE id_sesion=$id_sesion AND fechadeldia_tiraje = '$today'))*100 as calidad_primera";
    //obtenemos desempeño operando entregados+merma
    $etequery4 = "SELECT SUM(produccion_esperada) AS prod_esperada,SUM(merma_entregada) AS merma, SUM(buenos) AS prod_real  ,COUNT(desempenio) AS tirajes,SUM(produccion_esperada) AS esper FROM `tiraje` WHERE fechadeldia_tiraje='$today' AND id_sesion=$id_sesion AND tiempoTiraje IS NOT NULL";
    $etequery5 = "SELECT COALESCE((SELECT SUM(entregados) FROM tiraje WHERE id_sesion=$stationID AND fechadeldia_tiraje = '$today' AND tiempoTiraje IS NOT NULL)) as desempenio";
    ...
    $desempenio =($getEfec['prod_esperada']>0)? (($getEfec['prod_esperada']!=null)? ($getEfec['prod_real']/$getEfec['prod_esperada'])*100 : 0) :0;
    //echo $etequery3;
